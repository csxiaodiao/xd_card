...
module Wechat
  module ApiLoader
    def self.with(options)
      ...
      card_token_file = options[:js_token_file] || c.cardapi_ticket.presence || '/var/tmp/wechat_cardapi_ticket'
      ...
      if c.appid && c.secret && token_file.present?
        if type == 'mp'
          ...
          Wechat::Api.new(c.appid, c.secret, token_file, c.timeout, c.skip_verify_ssl, js_token_file, card_token_file)
        end
    ...
    private_class_method def self.loading_config!
      ...
      if defined?(::Rails)
        configs.each do |_, cfg|
          ...
          cfg[:cardapi_ticket] ||= Rails.root.try(:join, 'tmp/cardapi_ticket').try(:to_path)
    ...
    private_class_method def self.config_from_environment
      value = { appid: ENV['WECHAT_APPID'],
                ...
                cardapi_ticket: ENV['WECHAT_CARDAPI_TICKET'],
