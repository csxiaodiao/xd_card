...
module Wechat
  module Concern
    module Common
      ...
      # card
      def card_create(card_hash)
        post("create", JSON.generate(card_hash), base: Wechat::Api::CARD_BASE)
      end

      def paycell_set(card_id)
        post("paycell/set", JSON.generate({ card_id: card_id, is_open: true }), base: Wechat::Api::CARD_BASE)
      end

      def getpoilist
        post("getpoilist", JSON.generate({ buffer: { begin: 0, limit: 10 } }), base: Wechat::Api::POI_BASE)
      end
    end
  end
end
