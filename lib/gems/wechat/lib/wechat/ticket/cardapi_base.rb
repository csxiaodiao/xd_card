...
module Wechat
  module Ticket
    class CardapiBase
      attr_reader :client, :access_token, :cardapi_ticket_file, :access_ticket, :ticket_life_in_seconds, :got_ticket_at

      def initialize(client, access_token, cardapi_ticket_file)
        @client = client
        @access_token = access_token
        @cardapi_ticket_file = cardapi_ticket_file
      ...
      def read_ticket
        JSON.parse(File.read(cardapi_ticket_file))
      end

      def write_ticket(ticket_hash)
        File.write(cardapi_ticket_file, ticket_hash.to_json)
      end
